      ...
      <div class="row">
        ...
        <div class="col-lg-4 col-12">
          <div class="imageTitle">
            <img
              src="https://d33wubrfki0l68.cloudfront.net/a3cfa0f3d38012d1d2794ca6fc31b316542b98d8/8fe98/imgs/uscc.png"
            />
            <h3>كأس السوبر الاوروبي</h3>
          </div>
        </div>
        <div class="col-lg-8 col-12">
          <div class="info">
            ...
            <p>
              بطولة السوبر الاوروبي , تقام بين بطل دوري أبطال أوروبا و بطل
              الدوري الاوروبي في مباراة واحدة قبل بداية الموسم.
            </p>
            <hr />
            <h4>القوانين العامة</h4>
            <ul>
              <li>تقام مبارياتها على نظام صعوبة لاعب ممتاز</li>
              <li>مدة المباراة 10 دقائق على نظام اللعبة</li>
            </ul>
          </div>
        </div>
        <div class="col-12">
        ...
        >
          ...
          <div class="AllMatches">
            <div class="round">
              <h4>النهائي</h4>
              <div class="match">
                <span>مانشيستر سيتي</span>
                <span>1 - 4</span>
                <span>مانشيستر يونايتد</span>
              </div>
            </div>
          </div>
        </div>
...
.USC {
  ...
  .imageTitle {
    ...
    border-radius: 10px;
    overflow: hidden;
    box-shadow: 0 0 7px rgba($color: $usc, $alpha: 0.15);
    margin-bottom: 25px;
    img {
      width: 220px;
      height: 220px;
  ...
  .am {
    h5 {
      width: 80%;
      margin: 10px auto;
      box-shadow: 0 0 2px rgba($color: $usc, $alpha: 0.2);
      text-align: right;
      @include em {
        width: 85%;
      }
  ...
  .sbest {
    ...
    h5 {
      ...
      font-family: "Changa", sans-serif;
      font-weight: bold;
      box-shadow: 0 0 5px rgba($color: $usc, $alpha: 0.15);
      &:nth-of-type(4),
      &:nth-of-type(5),
      &:nth-of-type(6) {
        background: green;
...
export default {
  ...
  methods: {
    selectSeason() {
      ...
      // Get Season State
      db.get(`Seasons/${seNum}/USC`).then(respon => {
      ...
      db.get(`Seasons/${seNum}`).then(res => {
        // GET SEASONS MATCHES
        db.get(`Seasons/${seNum}/USC/matches`, { sort: ["-num"] })
          .then(response => {
        ...
        // GET SEASONS GS
        db.get(`Seasons/${seNum}/USC/GS`, { order: { type: "key" } }).then(
          res => {
            var num = 1;
        ...
        // GET SEASONS AS
        db.get(`Seasons/${seNum}/USC/AS`, { order: { type: "key" } }).then(
          res => {
            var num = 1;
            res.data.forEach(player => {
        ...
        // GET SEASONS BEST
        db.get(`Seasons/${seNum}/USC/BEST`).then(res => {
          res.data.forEach(player => {
    ...
    CreateNewSeason() {
      Swal.fire({
        ...
        Swal.fire({
          ...
          if (val.value) {
            if (res.value) {
              if (val.value.includes("ليستر سيتي")) {
                db.get(`playersData/hassan/toursStats/usc`).then(pd => {
                  var part = pd.data[0] + 1;
                  db.update(`playersData/hassan/toursStats/usc`, {
                    part
                  });
                });
              }

              if (val.value.includes("مانشيستر سيتي")) {
                db.get(`playersData/mo/toursStats/usc`).then(pd => {
                  var part = pd.data[0] + 1;
                  db.update(`playersData/mo/toursStats/usc`, {
                    part
                  });
                });
              }
              ...
              db.get("Seasons", {
                ...
                db.update(`Seasons/${SeNum}/USC`, {
                  state: false
                });

                // 1ST ROUND
                db.update(`Seasons/${SeNum}/USC/matches/round1`, {
                  num: 1
                });

                // 1ST ROUND MATCHES
                db.update(`Seasons/${SeNum}/USC/matches/round1/match1`, {
                  home: val.value[0],
                  homeScore: 0,
                  away: val.value[1],
                  awayScore: 0,
                ...
                // ADD GOAL SCORERS
                db.add(`Seasons/${SeNum}/USC/GS/1`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 1
                });

                db.add(`Seasons/${SeNum}/USC/GS/2`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 2
                });

                db.add(`Seasons/${SeNum}/USC/GS/3`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 3
                });

                db.add(`Seasons/${SeNum}/USC/GS/4`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 4
                });
                ...
                // ADD ASSIST MAKERS
                db.add(`Seasons/${SeNum}/USC/AS/1`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 1
                });

                db.add(`Seasons/${SeNum}/USC/AS/2`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 2
                });

                db.add(`Seasons/${SeNum}/USC/AS/3`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 3
                });

                db.add(`Seasons/${SeNum}/USC/AS/4`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 4
                });

                db.add(`Seasons/${SeNum}/USC/AS/5`, {
                  player: "لم يتم التعيين",
                  goals: 0,
                  _id: 5
                });
                ...
                // ADD SEASONS BEST
                db.add(`Seasons/${SeNum}/USC/BEST/FORWOARD`, {
                  player: "لم يتم التعيين",
                  _id: "FORWOARD"
                });

                db.add(`Seasons/${SeNum}/USC/BEST/CENTER`, {
                  player: "لم يتم التعيين",
                  _id: "CENTER"
                });

                db.add(`Seasons/${SeNum}/USC/BEST/DEFENDER`, {
                  player: "لم يتم التعيين",
                  _id: "DEFENDER"
                });

                db.add(`Seasons/${SeNum}/USC/BEST/GOALKEEPER`, {
                  player: "لم يتم التعيين",
                  _id: "GOALKEEPER"
                });

                var checker = setInterval(() => {
                  db.get(`Seasons/${SeNum}/USC`).then(res => {
                    if (res.data.length >= 5) {
                      clearInterval(checker);
                      Swal.fire(
                        "Done!",
                        "USC Add Successfully To Season " + SeNum,
                        "success"
                      ).then(() => {
                        setTimeout(() => {
                          location.reload();
    ...
    EndThisSeason() {
      Swal.fire({
        ...
        if (res.value) {
          var seNum = $(".select select").val();
          db.get(`Seasons/${seNum}/USC/matches`).then(res => {
            var checker = 0;
            ...
            if (checker > 0) {
              ...
              db.get(`Seasons/${seNum}/USC/matches/round1/match1`).then(res => {
                ...
                if (winner == "ليستر سيتي") {
                  firebase
                    .database()
                    .ref(`playersData/hassan/toursStats/usc`)
                    .transaction(data => {
                      data.win++;
                      return data;
                    })
                  ...
                  firebase
                    .database()
                    .ref(`playersData/mo/toursStats/usc`)
                    .transaction(data => {
                      data.win++;
                      return data;
                    })
  ...
  created() {
    window.scrollTo(0, 0);
    document.title = "ROPT | UEFA SUPER CUP";
    $(".preloader").show();
  ...
  mounted() {
    ...
    $(".AllMatches").on("click", ".match i", function() {
      ...
      Swal.fire({
        ...
        if (val.value) {
          ...
          db.update(`Seasons/${seNum}/USC/matches/${id}`, {
            homeScore,
            awayScore,
            state: true
          }).then(() => {
    ...
    $(".gs h5").on("dblclick", function() {
      ...
      Swal.fire({
        ...
        if (val.value) {
          ...
          db.update(`Seasons/${seNum}/USC/GS/${id}`, {
            player,
            goals
          });
        }
    ...
    $(".am h5").on("dblclick", function() {
      ...
      Swal.fire({
        ...
        if (val.value) {
          var player = val.value[0];
          var goals = val.value[1];
          db.update(`Seasons/${seNum}/USC/AS/${id}`, {
            player,
            goals
          });
        }
    ...
    $(".sbest p").on("dblclick", function() {
      ...
      Swal.fire({
        ...
        if (val.value) {
          var player = val.value[0];
          db.update(`Seasons/${seNum}/USC/BEST/${id}`, {
            player
          });
        }
      });
